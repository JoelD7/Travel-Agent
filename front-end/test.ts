...
// Pick a random prop of a chromosome
...
// Create a chromosome and select a random value to start with for each element
...
Chromosome.prototype.mutate = function () {
  if (Math.random() > this.mutationRate) return false;
...
Chromosome.prototype.mateWith = function (other) {
  var child1 = {};
  var child2 = {};
  // Pick random portion of each chromosone to keep
...
// This is typicially in the range of .1 - 1%
...
// Create new chromosomes and new ones from crossover
...
// Sort chromosomes based on score
...
Population.prototype.kill = function () {
  // Determine the number of chromosomes to kill off based on `elitism`
...
Population.prototype.run = function (threshold, noImprovement, lastScore, i) {
  ...
  if (!noImprovement) noImprovement = 0;
  if (!lastScore) lastScore = false;
  if (!i) i = 0;
  ...
  if (noImprovement < threshold) {
    ...
    if (i % 10 == 0) this.display(i, noImprovement);
    var scope = this;
    setTimeout(function () {
...
// Population props
// elitism determines who is fit to survive
